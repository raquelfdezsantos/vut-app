<x-app-layout>
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8">
        <h1 class="text-2xl font-bold mt-3 mb-3">{{ $property->name }}</h1>
        ...
        <div class="bg-white shadow sm:rounded-xl p-5 mt-4">
            ...
            <div class="text-sm text-gray-500 mb-4">
                {{ $property->address }} · {{ $property->city }} · Capacidad: {{ $property->capacity }} huéspedes
            </div>

            @auth
            <form method="POST" action="{{ route('reservations.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                @csrf
